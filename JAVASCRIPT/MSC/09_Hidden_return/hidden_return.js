...
const isHighRated1 = (imdbScore) => imdbScore > 8;

console.log(isHighRated());
console.log(isHighRated1());

//Синтаксис может показаться странным, но стоит к нему привыкнуть, как уже невозможно будет представить себе программирование 
...
console.log(isHighRated2());
//И это самый короткий (и одновременно очень понятный) способ записи подобной функции!
...
const imdbScores = [8.2, 6.3, 8.8, 5.8, 9.1];

const getHighRated1 = imdbScores.filter(function(imdbScore) {
  return imdbScore > 8;
})
...
const getHighRated2 = imdbScores.filter(imdbScore => imdbScore > 8);
...
const result2 = testnets.find(function(testnet) {
...
const result3 = testnets.find(testnet => testnet === "Rinkeby");
...
const doubled4 = numbers.map(number => number * 2);
