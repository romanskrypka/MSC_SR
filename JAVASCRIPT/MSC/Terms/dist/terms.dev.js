...
console.log(faceControl(age)); //Для проверки на равенство мы использовали тройное равно ===, которое рекомендуем использовать всегда.
...
/*export - деактивировал, потому что не откуда экспортировать*/
...
console.log(getNextAge(age)); //Practice - 25
...
console.log(getExcerpt(text)); //!Возврат true/false - Return true/false
...
console.log(isSupercentenarian(age2)); //Но выражение age >= 110 и так превращается в true или false в зависимости от возраста.
...
console.log(isSupercentenarian2(age2)); //Это будет работать только с возвратом логических значений из функций. Со всеми другими значениями без if/else не обойтись.
...
console.log(isSupercentenarian3(age2)); //Practice - 26
...
console.log(canRide(height)); //!Проверка на четность
